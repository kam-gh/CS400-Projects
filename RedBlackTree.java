...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    ...
    // case 0: throw exception when necessary
    if (parent == null || child == null || !parent.equals(child.context[0])) {
      throw new IllegalArgumentException();
    }

    // case 1: Right Rotation (left children)
    if (!child.isRightChild()) {

      // set nodes to variables
      Node<T> grandparent = parent.context[0];
      Node<T> childRight = child.context[2];

      // rearranging tree
      child.context[2] = parent;
      parent.context[0] = child;
      parent.context[1] = childRight;
      child.context[0] = grandparent;

      // check if subtree exists
      if (childRight != null) {
        childRight.context[0] = parent;
      }

      // check if root; if not, find out if parent is left or right child
      if (grandparent == null) {
        System.out.println("Grandparent is null (Right Rotation).");
        this.root = child;
      } else {
        if (parent.equals(grandparent.context[1])) {
      ...
      // case 2: Left Rotation (right child)
      Node<T> grandparent2 = parent.context[0];
      Node<T> childLeft = child.context[1];

      // rearranging tree
      child.context[1] = parent;
      parent.context[0] = child;
      parent.context[2] = childLeft;
      child.context[0] = grandparent2;

      // check if subtree exists
      if (childLeft != null) {
        childLeft.context[0] = parent;
      }

      // check if root; if not, find out if parent is left or right child
      if (grandparent2 == null) {
        System.out.println("Grandparent is null (Left Rotation).");
        this.root = child;
      } else {
        if (parent.equals(grandparent2.context[1])) {
          grandparent2.context[1] = child;
        } else {
          grandparent2.context[2] = child;
        }
      }
    }
  }
  ...
  /**
   * This tester method tests the rotate() method if the grandparent is the root of the tree. For
   * both right and left rotation
   * 
  ...
  public static boolean test1() {
    ...
    Node<Integer> left = root.context[1];
    Node<Integer> leftmost = left.context[1];

    System.out.println(root.data);
    System.out.println(left.data);
    System.out.println(leftmost.data);

    System.out.println("before: " + tree.toLevelOrderString());
    System.out.println("before: " + tree.toInOrderString());

    tree.rotate(left, root);

    System.out.println("after: " + tree.toLevelOrderString());
    System.out.println("after: " + tree.toInOrderString());
    System.out.println("root now: " + tree.root.data);

    if (!tree.toLevelOrderString().trim().equals("[ 4, 3, 5 ]")) {
      return false;
    }
    ...
    Node<Integer> root2 = tree2.root;
    Node<Integer> right = root2.context[2];
    Node<Integer> rightmost = right.context[2];

    System.out.println(root2.data);
    System.out.println(right.data);
    System.out.println(rightmost.data);

    System.out.println("before: " + tree2.toLevelOrderString());
    System.out.println("before: " + tree2.toInOrderString());

    tree2.rotate(right, root2);

    System.out.println("after: " + tree2.toLevelOrderString());
    System.out.println("after: " + tree2.toInOrderString());
    System.out.println("root now: " + tree2.root.data);

    if (!tree2.toLevelOrderString().trim().equals("[ 2, 1, 3 ]")) {
      return false;
    }

    if (tree2.root.data != 2) {
      return false;
    }
  ...
  /**
   * This test method checks that the rotate() method works if the rotation is within the tree.
   * 
   * @return true if the test passed, false if not.
   */
  public static boolean test2() {

    // test internal right rotation
    RedBlackTree<Integer> tree = new RedBlackTree<>();
    ...
    Node<Integer> root = tree.root;
    Node<Integer> left = root.context[1];
    Node<Integer> leftAgain = left.context[1];
    Node<Integer> leftmost = leftAgain.context[1];
    ...
    tree.rotate(leftAgain, left);

    System.out.println("after: " + tree.toLevelOrderString());
    System.out.println("after: " + tree.toInOrderString());
    System.out.println("TEST 2-root now: " + tree.root.data);

    if (!tree.toLevelOrderString().trim().equals("[ 5, 3, 2, 4 ]")) {
      return false;
    }

    // test internal left rotation
    RedBlackTree<Integer> tree2 = new RedBlackTree<>();
    ...
    Node<Integer> root2 = tree2.root;
    Node<Integer> right = root2.context[2];
    Node<Integer> rightAgain = right.context[2];
    Node<Integer> rightmost = rightAgain.context[2];
    ...
    tree2.rotate(rightAgain, right);

    System.out.println("after: " + tree2.toLevelOrderString());
    System.out.println("after: " + tree2.toInOrderString());
    System.out.println("root now: " + tree2.root.data);

    if (!tree2.toLevelOrderString().trim().equals("[ 1, 3, 2, 4 ]")) {
      return false;
    }
  ...
  /**
   * This test method checks that the rotate() method works if the tree has invalid input.
   * 
   * @return true if the test passed, false if not.
   */
  public static boolean test3() {

    RedBlackTree<Integer> badTree = new RedBlackTree<>();
    badTree.insert(5);
    badTree.insert(4);
    badTree.insert(3);
    badTree.insert(2);

    Node<Integer> root = badTree.root;
    Node<Integer> left = root.context[1];
    Node<Integer> leftAgain = left.context[1];
    Node<Integer> leftmost = leftAgain.context[1];
    Node<Integer> bad = null;

    // bad input
    try {
      badTree.rotate(root, leftmost);
